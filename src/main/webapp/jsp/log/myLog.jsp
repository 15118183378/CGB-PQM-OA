...
<body>
	<input id="logTypeId" name="logTypeId" type="hidden" value="${logTypeId }" />
	<div align="left">
				...
				<td style="border-style: none; border-width: medium">
					<form id="createOrg" class="contact_form" action="#" method="post"
						...
						<ul>
							...
							<!-- <li><label for="time">开始时间：</label>
								<input type="text" name="startTime" placeholder="1990-1-1"/>
							</li>
							<li><label for="time">结束时间：</label>
								<input type="text" name="endTime" placeholder="1990-1-1"/>
							</li> -->
							<div id="body-wrapper">
								<div id="main-content">
											...
											<tbody id="tbodyCompany">
												<c:forEach items="${logs }" var="log">
													<tr class="log${log.id }">
														...
														<td><c:if test="${log.status == 1 }">已阅</c:if><c:if test="${log.status == 0 or empty log.status }">未阅</c:if></td>
														<td><a
															href="${pageContext.request.contextPath }/log/selectByLogId/${log.id}?type='my'"
															title="Edit">查看</a></td>
													</tr>
													<% num++; %>
												</c:forEach>
...
<script type="text/javascript">
	...
	$('#logType').change(function() {
		window.location.href = "${pageContext.request.contextPath }/log/getMyLog/" + $("#logType").val();
	});
</script>
</html>